        ...
        NavigationStack(path: $path) {
            Group {
                if activities.isEmpty {
                    ContentUnavailableView("Create your first work by tapping on the \(Image(systemName: "plus.circle.fill")) button at the top.", image: "launchScreen")
                } else {
                    List(activities) { activity in
                        NavigationLink(value: activity){
                            HStack {
                                ...
                                Spacer()
                                Text("^[\(activity.workouts.count) works due](inflect: true)")
                                    .font(.caption)
